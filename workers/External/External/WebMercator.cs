...
namespace External
{
    /// SOURCE: https://gist.github.com/nagasudhirpulla/9b5a192ccaca3c5992e5d4af0d1e6dc4
    ...
    class WebMercator
    {
        ...
        public static PointShape LatLonToMeters(double lat, double lon)
        {
            var p = new PointShape();
        ...
        //Converts XY PointShape from (Spherical) Web Mercator EPSG:3785 (unofficially EPSG:900913) to lat/lon in WGS84 Datum
        public static PointShape MetersToLatLon(PointShape m)
        {
            var ll = new PointShape();
        ...
        public static PointShape PixelsToMeters(PointShape p, int zoom)
        {
            var res = Resolution(zoom);
            var met = new PointShape();
        ...
        public static PointShape MetersToPixels(PointShape m, int zoom)
        {
            var res = Resolution(zoom);
            var pix = new PointShape();
        ...
        public static Tile PixelsToTile(PointShape p)
        {
            var t = new Tile();
        ...
        public static PointShape PixelsToRaster(PointShape p, int zoom)
        {
            var mapSize = TileSize << zoom;
            return new PointShape(p.X, mapSize - p.Y);
        }

        //Returns tile for given mercator coordinates
        public static Tile MetersToTile(PointShape m, int zoom)
        {
        ...
        public static RectangleShape TileBounds(Tile t, int zoom)
        {
            var min = PixelsToMeters(new PointShape(t.X * TileSize, t.Y * TileSize), zoom);
            var max = PixelsToMeters(new PointShape((t.X + 1) * TileSize, (t.Y + 1) * TileSize), zoom);
            return new RectangleShape(min, max);
        }

        //Returns bounds of the given tile in latutude/longitude using WGS84 datum
        //public static RectangleShape TileLatLonBounds(Tile t, int zoom)
        //{
        //    var bound = TileBounds(t, zoom);
        //    var min = MetersToLatLon(new PointShape(bound., bound.Top));
        //    var max = MetersToLatLon(new PointShape(bound.Right, bound.Bottom));
        //    return new RectangleShape(min, max);
        //}
        ...
        public static string LatLonToQuadTree(PointShape latLon, int zoom)
        {
            PointShape m = LatLonToMeters(latLon.Y, latLon.X);
        ...
        public static List<string> GetQuadTreeList(int zoom, PointShape latLonMin, PointShape latLonMax)
        {
            if (latLonMax.Y < latLonMin.Y || latLonMax.X < latLonMin.X)
                return null;

            PointShape mMin = LatLonToMeters(latLonMin.Y, latLonMin.X);
            Tile tmin = MetersToTile(mMin, zoom);
            PointShape mMax = LatLonToMeters(latLonMax.Y, latLonMax.X);
